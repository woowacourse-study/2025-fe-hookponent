...
export type MediaCondition =
  | { key: 'max-width' | 'min-width' | 'max-height' | 'min-height'; value: PxNumber }
  | { key: 'aspect-ratio' | 'min-aspect-ratio' | 'max-aspect-ratio'; value: RatioTuple }
  | { key: 'min-resolution' | 'max-resolution'; value: { amount: number; unit?: ResolutionUnit } }
 ...
 * ```
 */
export function useMediaQuery(condition: MediaCondition | MediaCondition[]): boolean | boolean[] {
  const toStrings = (q: MediaCondition | MediaCondition[]) => {
    const list = Array.isArray(q) ? q : [q];
    return list.map((item) => (typeof item === 'string' ? item : toMediaString(item)));
  };

  const getInitialMatches = () => {
    if (typeof window === 'undefined') {
      return Array.isArray(condition) ? Array<boolean>(condition.length).fill(false) : false;
    }
  ...
  useEffect(() => {
    ...
    if (Array.isArray(queries)) {
      ...
      return () => mediaList.forEach((media) => media.removeEventListener('change', listener));
    } else {
